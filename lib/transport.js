    ...
    connect() {
        ...
        this.client.on('connect', () => {
            ...
            this.client.subscribe(this.subscribeTopics, { qos: 1 }, (err, granted) => {
                ...
                if (this.log) {
                    this.log('subscribed', this.url, granted);
                }
                // QoS 128 in the SUBACK means the broker refused the subscription (ACL)
                const refused = (granted || []).filter((g) => g.qos === 128).map((g) => g.topic);
                if (refused.length) {
                    this.emit('error', new RequestError('Broker ' + this.url + ' refused subscription to ' + refused.join(', ')));
                }
                this.emit('connect');
            });
        });
    ...
    request(device, method, namespace, payload, timeout) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                ...
                reject(timeoutError('Timeout waiting for ' + namespace + ' response from ' + device.uuid +
                    ' via ' + this.url + ' (device offline, wrong UUID or on another broker?)'));
            }, timeout || this.timeout);
            this.pending.set(msg.header.messageId, { resolve, reject, timer, method, namespace });
            const topic = '/appliance/' + device.uuid + '/subscribe';
            if (this.log) {
                this.log('->', topic, msg);
            }
    ...
    async request(device, method, namespace, payload, timeout) {
        ...
        try {
            ...
            if (err.name === 'TimeoutError') {
                throw timeoutError('Timeout waiting for ' + namespace + ' response from ' + device.host);
            }
