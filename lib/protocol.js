...
const NS = {
    ...
    ELECTRICITYX: 'Appliance.Control.ElectricityX',
    CONSUMPTIONH: 'Appliance.Control.ConsumptionH',
...
/**
 * Convert Appliance.Control.ElectricityX (newer devices like MOP320, EM06).
 * Units: current mA, voltage mV, power mW, mConsume Wh.
 * The payload can contain a single channel object or a list of channels.
 * Returns an array of per-channel values (possibly empty).
 */
function parseElectricityX(payload) {
    const e = payload && payload.electricity;
    if (!e) {
        return [];
    }
    ...
    const round = (v, d) => Math.round(v * Math.pow(10, d)) / Math.pow(10, d);
    return list
        .filter((c) => c && typeof c === 'object' && ('power' in c || 'current' in c || 'voltage' in c))
        .map((c) => {
            const out = {
                channel: c.channel || 0,
                power: round((c.power || 0) / 1000, 3),
                voltage: round((c.voltage || 0) / 1000, 1),
                current: round((c.current || 0) / 1000, 3)
            };
            if (typeof c.mConsume === 'number') {
                out.energy = c.mConsume;
            }
            if (typeof c.factor === 'number') {
                out.factor = c.factor;
            }
            return out;
        });
